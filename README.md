...
* **v11.1 (Current):** **Market Search & Stability.** ABD borsaları (NYSE, NASDAQ, AMEX) için genişletilmiş arama filtresi eklendi. Analiz sırasında oluşan "innerHTML" crash hatası temizlendi. `cdn.jsdelivr.net` için CSP tanımları güncellendi. Eksik tablolar için `background_connectivity_test.py` ve `supabase_setup.sql` hazırlandı.
